...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(GPIO1_PIN), band38, CHANGE);
  
  setDefaults();
}

void band38() {
  digitalWrite(BAND38_TX_ENn_PIN, digitalRead(GPIO1_PIN));
}
...
void doCommand(){
  switch(activeBuffer[0]) {
    ...
    case CMD_CONFGPIO10:  {
        configGPIO10(activeBuffer[1], activeBuffer[2]);
        break;
      }    
    case CMD_SETGPIO10:  {
        setGPIO10(activeBuffer[1], activeBuffer[2]);
        break;
      }
    case CMD_GETGPIO10:  {
        char pin = activeBuffer[1];
        clearBuffer();
        activeBuffer[1] = getGPIO10(pin);
        break;
      }
    case CMD_GET_INFO: {
        clearBuffer();  
